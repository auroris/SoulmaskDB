 ...
 * absent or empty):
 *
 *   displayName     — player-set label.
 *                     NPCs:       CustomMingZi (TextProperty) — e.g. "[Aleena] Torven"
 *                     Buildings:  JianZhuDisplayName (TextProperty) — e.g. "Alfheimr"
 *                     Signs:      CurGaoShiString (StrProperty)
 *   customNote      — NPC role comment (CustomBeiZhu StrProperty) — e.g. "Craftsman"
...
export function collectFacts(decoded) {
  ...
  for (const p of props) {
    switch (p.name) {
      case 'CustomMingZi': {
        // NPC player-set display name, e.g. "[Aleena] Torven"
