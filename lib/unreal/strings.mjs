...
function collectFromValue(value, path, out) {
  ...
  if (value instanceof StructValue) {
    // Known binary structs (Vector, Quat, …) have numeric POJO values; only
    // recurse when value is a nested property array.
    if (Array.isArray(value.value)) collectFromPropertyArray(value.value, path, out);
    return;
  }
