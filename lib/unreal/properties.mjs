...
export class ArrayValue {
  constructor({ elements = [], innerTag = null, trailing = null } = {}) {
    this.elements = elements;
    this._arrayInnerTag = innerTag;
    // Soulmask-specific tail for ArrayProperty<ObjectProperty>: see
    // readObjectArrayTrailing in properties.mjs. Null when absent.
...
function readObjectValue(cursor, sizeHint) {
  ...
  try {
    ...
    if (cursor.pos() - start >= sizeHint) {
      return new ObjectRef({ kind, path: pathFS.value, pathIsNull: pathFS.isNull });
    }
...
function readArrayValue(cursor, tag, sizeHint) {
  ...
  // ObjectProperty elements have variable wire shapes (kind-only,
  // kind+path, kind+path+classPath, or full with embedded stream). Without
  // a per-element bound, `readArrayElement` would unconditionally read all
  // four fields and overrun arrays whose elements omit the trailing fields
  // (e.g. ChengHaoList with numElements=1 and a kind+path-only element —
  // the FString-after-path then gets misread from the next property's tag
  // and the cursor goes off the rails). Pass each element the remaining
  // share of the array's size budget so it can early-exit when the budget
  // is exhausted, mirroring `readObjectValue`'s bounded reads at the top
  // level. The equal-share split is heuristic but exact for
  // numElements=1 (covers every observed failing case in world.db) and
  // a reasonable upper bound for homogeneous multi-element arrays.
  ...
  for (let i = 0; i < numElements; i++) {
    ...
    if (isObj) {
      ...
      const elemsLeft = numElements - i;
      elemSizeHint = elemsLeft > 0 ? Math.max(0, Math.floor(remaining / elemsLeft)) : 0;
    }
    elements.push(readArrayElement(cursor, innerType, elemSizeHint));
  }

  // Soulmask-specific tail for ArrayProperty<ObjectProperty>: when the array's
  // tag.size budget exceeds what the elements consumed, the leftover bytes
  // are a self-describing binary section (see readObjectArrayTrailing).
  let trailing = null;
  if (isObj && cursor.pos() < endOff) {
    trailing = readObjectArrayTrailing(cursor, endOff);
  }

  return new ArrayValue({ elements, trailing });
}
 ...
 * the layout
 *
 *   [3 × float32]                  origin (always observed (0,0,0))
 *   repeated until budget exhausted:
 *     [u32 stride] [u32 count]     section header
 *     [count × stride bytes]       opaque section payload
 *
 * Empirically the sections come in triples: (a) stride=64, count=N world
 * 4×4 transform matrices for the placed building pieces; (b) stride=4,
 * count=N per-piece u32 IDs; (c) stride=64, count=M local 4×4 transforms
 * for the prototype shapes (typically M = N or N+1). We don't interpret
 * the payload bytes here — that's a renderer concern. The decoder only
 * needs the structural framing to (1) consume exactly tag.size bytes so
 * the outer stream doesn't trip its sizeMismatch handler, and (2) preserve
 * the bytes for byte-identical round-trip on write.
 *
 * If the bytes don't match the format, fall back to `{ _raw, _parseError }`
 * — the writer just emits `_raw` verbatim and round-trip still survives.
 */
function readObjectArrayTrailing(cursor, endOffset) {
  const start = cursor.pos();
  try {
    if (endOffset - start < 12) throw new Error('not enough bytes for origin');
    const origin = {
      x: cursor.readFloat32(),
      y: cursor.readFloat32(),
      z: cursor.readFloat32(),
    };
    const sections = [];
    while (cursor.pos() < endOffset) {
      ...
      if (count > 100000) throw new Error(`implausible count ${count}`);
...
function writeObjectArrayTrailing(writer, trailing) {
  ...
  writer.writeFloat32(trailing.origin.x);
  writer.writeFloat32(trailing.origin.y);
  writer.writeFloat32(trailing.origin.z);
  ...
  switch (innerType) {
    ...
    case 'WSObjectProperty': {
      ...
      return new ObjectRef({
        ...
        path: pathFS.value, pathIsNull: pathFS.isNull,
        classPath: classPathFS.value, classPathIsNull: classPathFS.isNull,
        embedded: stream.properties, terminated: stream.terminated,
      });
    }
    case 'SoftObjectProperty':
    case 'SoftClassProperty':
