...
export class ArrayValue {
  constructor({ elements = [], innerTag = null, trailing = null, perElementTrailings = null } = {}) {
    this.elements = elements;
    this._arrayInnerTag = innerTag;
    // Soulmask trailing data for ArrayProperty<ObjectProperty>:
    //   _perElementTrailings — per-prototype placement-binary blocks
    //     interleaved with elements (JianZhuInstYuanXings layout). Parallel
    //     array to `elements`; entries may be null when an element has no
    //     trailing of its own.
    //   _trailing — legacy "single trailing block AFTER all elements"; only
    //     populated when bytes remain at the array's tail after the
    //     per-element pass. Most modern rows use per-element trailings only.
...
function readArrayValue(cursor, tag, sizeHint) {
  ...
  // placed-piece transforms, ids and aux data for THAT prototype. The
  // per-element trailing is detected by peeking 8 zero bytes + a leading
  // stride=64 — see tryReadObjectArrayPerElementTrailing. Verified in-game:
  // adding a new prototype increments numElements by 1 and inserts a new
  // (kind=3 + 8B header + 3 sections) block after the previous element's
  // sections (2026-05-18 test bonfire experiment).
  ...
  for (let i = 0; i < numElements; i++) {
    ...
    if (isObj) {
      const t = tryReadObjectArrayPerElementTrailing(cursor, endOff);
      if (t) anyPerElementTrailing = true;
      perElementTrailings.push(t); // null when no per-element trailing for this element
    }
  }

  // If ANY element had a per-element trailing block, store the whole
  // parallel array (with nulls for elements that don't have one).
  // Otherwise fall back to the legacy "single trailing block after all
  // elements" parser for any leftover bytes — this catches whatever
  // residual non-JianZhuInstYuanXings ObjectProperty arrays look like.
  let trailing = null;
  if (isObj && cursor.pos() < endOff) {
    trailing = readObjectArrayTrailing(cursor, endOff);
  }
...
/**
 * Try to read a per-element trailing block for ArrayProperty<ObjectProperty>
 * (specifically the JianZhuInstYuanXings building-zone layout). Block format:
 *
 *   [8 bytes zero header]
 *   [u32 stride=64] [u32 count]  [count×64 bytes transforms]
 *   [u32 stride=4]  [u32 count]  [count×4  bytes piece-ids]
 *   [u32 stride=64] [u32 count]  [count×64 bytes auxiliary]
 *
 * Returns the parsed { header, sections } or null when the bytes at the
 * cursor don't match (no header zeros, wrong stride, etc.). The cursor is
 * rolled back to the start position on failure so the caller's element
 * loop can continue without disruption.
 */
function tryReadObjectArrayPerElementTrailing(cursor, endOff) {
  const start = cursor.pos();
  // Minimum size: 8B header + 3 × (8B section header) = 32B (sections may
  // have count=0 with no payload).
  if (endOff - start < 32) return null;
  // Header: all 8 bytes must be zero.
  for (let i = 0; i < 8; i++) {
    if (cursor.bytes[start + i] !== 0) return null;
  }
  // Section 0 stride at offset +8: must be 64.
  const stride0 = cursor.dv.getUint32(start + 8, true);
  if (stride0 !== 64) return null;

  // Looks like a valid per-element trailing. Read it.
  try {
    cursor.skip(8);
    const header = cursor.bytes.subarray(start, start + 8).slice();
    const sections = [];
    const expectedStrides = [64, 4, 64];
    for (let i = 0; i < 3; i++) {
      ...
      if (stride !== expectedStrides[i]) throw new Error(`section ${i} stride ${stride} != ${expectedStrides[i]}`);
      ...
      const data = cursor.readBytes(dataBytes).slice();
      sections.push({ stride, count, data });
    }
    return { header, sections };
  } catch {
    // Roll back — the heuristic was wrong about this being a trailing block.
    cursor.seek(start);
    return null;
  }
}
...
/**
 * Trailing binary section that follows the elements of certain
 * ArrayProperty<ObjectProperty> values (observed only on
 * JianZhuInstYuanXings so far — building instance prototypes for boats,
 * rafts, and built structures). Lives within the array tag's size budget
 * but is NOT a tagged property stream: it's a packed binary format with
 * the layout
 *
 *   [8 bytes]                      header — always observed as zeros.
 *                                  Possibly two i32 reserved fields or a
 *                                  paired (count, capacity) prefix; the
 *                                  semantics don't matter for round-trip
 *                                  since we capture and replay verbatim.
 *   repeated until budget exhausted:
 *     [u32 stride] [u32 count]     section header
 *     [count × stride bytes]       opaque section payload
 *
 * Empirically the sections come in triples for placed-building zones:
 *   (a) stride=64, count=N — N world 4×4 row-major FMatrix44 transforms
 *       for placed building pieces (translation at floats[12..14]).
 *   (b) stride=4,  count=N — N per-piece u32 piece-ids.
 *   (c) stride=64, count=N — N per-piece auxiliary 64-byte structs
 *       (looks like an axis-aligned bounding box + scale-ish floats —
 *       see the test-bonfire dump for a worked example with one piece).
 *
 * Verified via in-game experiment with one stone foundation
 * (serial=43178, 2026-05-18): header is 8 zero bytes, then three
 * sections (stride=64/count=1, stride=4/count=1, stride=64/count=1)
 * totalling exactly 164 bytes. The prior 12-byte "origin" guess
 * mis-aligned bytes 8..11 (the first stride) as `origin.z`, then read
 * `count_0` as the next stride and the first 4 bytes of the transform
 * matrix as a count — producing the "implausible count 3204775516"
 * fallback that left 15 BP_JianZhuPianQu rows as OpaqueValue.
 *
 * The decoder only needs the structural framing to (1) consume exactly
 * tag.size bytes so the outer stream doesn't trip its sizeMismatch
 * handler, and (2) preserve the bytes for byte-identical round-trip on
 * write. If the bytes don't match the format, fall back to
 * `{ _raw, _parseError }` — the writer emits `_raw` verbatim.
 */
function readObjectArrayTrailing(cursor, endOffset) {
  const start = cursor.pos();
  try {
    if (endOffset - start < 8) throw new Error('not enough bytes for header');
    const header = cursor.readBytes(8).slice();
    const sections = [];
    while (cursor.pos() < endOffset) {
      if (endOffset - cursor.pos() < 8) throw new Error('not enough bytes for section header');
      const stride = cursor.readUint32();
      const count  = cursor.readUint32();
      if (stride === 0 || stride > 4096) throw new Error(`implausible stride ${stride}`);
      if (count > 1_000_000) throw new Error(`implausible count ${count}`);
      const dataBytes = stride * count;
      if (cursor.pos() + dataBytes > endOffset) throw new Error('section overshoots budget');
      const data = cursor.readBytes(dataBytes).slice();
      sections.push({ stride, count, data });
    }
    if (cursor.pos() !== endOffset) throw new Error('sections did not exhaust budget');
    return { header, sections };
  } catch (e) {
    cursor.seek(start);
    const raw = cursor.readBytes(endOffset - start).slice();
    return { _raw: raw, _parseError: e.message };
  }
}

function writeObjectArrayTrailing(writer, trailing) {
  if (trailing._raw) { writer.writeBytes(trailing._raw); return; }
  writer.writeBytes(trailing.header);
  for (const s of trailing.sections) {
...
function writeArrayValue(writer, tag, value) {
  ...
  if (innerType === 'StructProperty') {
    ...
    if (value._trailing) writeObjectArrayTrailing(writer, value._trailing);
    return;
  }
  const perEl = value._perElementTrailings;
  for (let i = 0; i < value.elements.length; i++) {
    writeArrayElement(writer, innerType, value.elements[i]);
    if (perEl && perEl[i]) writeObjectArrayTrailing(writer, perEl[i]);
  }
  if (value._trailing) writeObjectArrayTrailing(writer, value._trailing);
}
