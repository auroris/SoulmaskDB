...
import { collectGuids } from '../unreal/refs.mjs';
...
function extractManifest(decoded) {
  // Cross-row references. Currently only Guid struct values
  // (ZhuRenGuid / GongHuiGuid / SelfUid / JianZhuBuilderUid / …) —
  // every entry resolves to another row whose `SelfUid` equals `guid`.
  // A future ReferencesService keyed by guid pairs these up without
  // touching the main-thread property tree. Future kinds: ObjectRef
  // instance suffixes, SoftObjectRef asset paths, Steam IDs.
  const guidRefs = collectGuids(decoded);
  const refs = new Array(guidRefs.length);
