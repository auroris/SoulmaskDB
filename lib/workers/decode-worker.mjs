...
import { collectGuids, collectObjRefs } from '../unreal/refs.mjs';
import { collectFacts } from '../unreal/facts.mjs';
import { Lz4Service } from '../lz4-wasm/lz4-service.mjs';
import { bindLz4 } from '../unreal/blob.mjs';

// Each worker context has its own lz4 wasm instance — wasm modules aren't
// shareable across threads. Boot it before announcing ready so the first
// decode-batch can call lz4Decompress without racing the wasm load.
...
// Announce readiness AFTER the message handler is wired AND lz4 has booted.
// The pool waits for this before dispatching the first batch — at least one
// browser we tested drops `worker.postMessage(...)` calls made before the
// worker module finishes evaluating, despite the spec saying they should
// be queued. The cost of this handshake is one postMessage per worker per
...
function extractManifest(decoded) {
  // Cross-row references — two flavors today:
  //   * Guid struct values (ZhuRenGuid / GongHuiGuid / SelfUid /
  //     JianZhuBuilderUid / …). Resolved by ReferencesService against
  //     each row's identity-guid index.
  //   * ObjectRef paths that look like actor instances
  //     (`/Game/.../BP_FooBar_C_<id>`). Resolved by ReferencesService
  //     against actor_name, since the Unreal path stored in an
  //     ObjectRef equals the target row's actor_name verbatim. These
  //     capture relationships that the GUID layer misses — most
  //     notably NPC pawn → inventory via `HBindBGCompActor`.
  //  Future kinds: SoftObjectRef asset paths, Steam IDs.
  const guidRefs = collectGuids(decoded);
  const objRefs  = collectObjRefs(decoded);
  const refs = new Array(guidRefs.length + objRefs.length);
  let idx = 0;
  for (let i = 0; i < guidRefs.length; i++) {
    refs[idx++] = { kind: 'guid', guid: guidRefs[i].guid, path: guidRefs[i].path };
  }
  for (let i = 0; i < objRefs.length; i++) {
    refs[idx++] = { kind: 'objref', path: objRefs[i].path, targetPath: objRefs[i].targetPath };
  }
  ...
  if (decoded.kind === 'unreal-properties') {
    const facts = collectFacts(decoded);
    return {
      ...base,
